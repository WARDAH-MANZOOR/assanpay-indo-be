...
const dalalMartPayoutController = async (req: Request, res: Response) => {
    ...
    try {
        ...
        payment_method = formatProvider(payment_method)
        ...
        if (!token?.access_token) {
            ...
            await prisma?.disbursement.create({
                data: {
                    ...
                    provider: payment_method == "bKash" ? PROVIDERS.BKASH : PROVIDERS.NAGAD,
                    status: "failed",
                    response_message: "No Token Recieved",
                    providerDetails: {
                        id: 1,
            ...
            return res.status(500).json({ status: 'error', message: 'No token recieved', data: {} });
        }

        let withdrawRequest = new FormData();
        withdrawRequest.append('trxId', data.merchant_custom_order_id);
        withdrawRequest.append('amount', amount);
        withdrawRequest.append('disburse_type', 'MFS');
        withdrawRequest.append('receiver_bank_info', `{"bank_name":"MFS","branch":"${payment_method}","ac_no":"${phone}"}`);
        withdrawRequest.append('remarks', 'Test');
        console.log("Request: ", withdrawRequest)
        let config = {
            method: 'post',
            maxBodyLength: Infinity,
            url: 'https://live.dalalmart.com/api/paymentroute/disbursement-init',
            headers: {
                'Accept': 'application/json',
                'Authorization': `Bearer ${token?.access_token}`,
                ...withdrawRequest.getHeaders()
            },
            data: withdrawRequest
        };

        let withdrawResponse = await axios.request(config)
            .then((response) => {
                return response.data;
            })
            .catch((error) => {
        ...
        if (withdrawResponse.code === 200 || withdrawResponse.code === 201) {
            ...
            delete withdrawResponse?.data["charge"]
            delete withdrawResponse?.data["total"]
            delete withdrawResponse?.data["current_balance"]
            return res.status(200).json({
                status: 'success', data: {
                    ...
                    "data": {
                        "status": "Pending",
                        "payment_id": "",
                        "amount": disbursement?.transactionAmount,
                        "currency": "BDT",
                        "payment_method": payment_method == "bKash" ? PROVIDERS.BKASH : PROVIDERS.NAGAD,
                        "withdraw_number": phone,
                        "reference": order_id,
                        "note": null
                    }
            ...
            await prisma?.disbursement.create({
                data: {
                    ...
                    provider: payment_method == "bKash" ? PROVIDERS.BKASH : PROVIDERS.NAGAD,
                    status: "failed",
                    response_message: withdrawResponse?.message,
                    providerDetails: {
                        id: 1,
                        sub_name: payment_method == "bkash" ? PROVIDERS.BKASH : PROVIDERS.NAGAD
                    }
            ...
            balanceDeducted = false;
            return res.status(withdrawResponse.code).json({ status: 'error', message: withdrawResponse?.message, data: {} });
        }
    } catch (error: any) {
        console.error(error);
        if (balanceDeducted) {
